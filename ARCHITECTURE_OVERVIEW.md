...
9. During editing, the editor keeps `objetos`, `secciones`, `rsvp`, and `gifts` in React state. `useBorradorSync` debounces writes back to Firestore for drafts or to `saveTemplateEditorDocument` for template sessions. Immediate flushes before critical actions are centralized in `flushEditorPersistenceBeforeCriticalAction`: template sessions prefer a direct flush bridge when available, while draft sessions use the `editor:draft-flush:request` / `editor:draft-flush:result` event flow.
10. When the dashboard generates preview, it first forces that immediate flush. The flush helper can capture a compatibility snapshot via `readEditorRenderSnapshot`. The dashboard then re-reads the draft from Firestore or the template editor document from the admin API, and if that boundary snapshot (or the current adapter snapshot) exists, it overwrites the re-read payload with that live render state before HTML generation.
