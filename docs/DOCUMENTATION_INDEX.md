...
| Transactional email sandbox smoke test | [TRANSACTIONAL_EMAIL_SANDBOX_RUNBOOK.md](operations/TRANSACTIONAL_EMAIL_SANDBOX_RUNBOOK.md) | Phase 1 validated end to end by the operator: private IAM smoke function, React Email, SES v2 sandbox, verified IAM policy, Secrets and offline checks. Manual smoke/disable procedure; temporary STS code removed, remote retirement pending. No business consumers or Firebase Auth migration; production remains blocked. |
| Backend configuration isolation | [BACKEND_CONFIGURATION_ISOLATION.md](operations/BACKEND_CONFIGURATION_ISOLATION.md) | Dotenv exposure evidence, per-function Secret bindings and source exclusions. Payments migration and renewed key pair validated in production; exposed webhook Secret v1 rotation and email source isolation remain pending. Manual rotation prerequisites are retained. |
| Payments production migration | [PAYMENTS_CODEBASE_PREPARATION.md](operations/PAYMENTS_CODEBASE_PREPARATION.md) | Completed 102 + 3 partition; production payment/webhook/publication acceptance confirmed by the operator on 2026-09-24. New Public Key + Access Token v2 validated; exposed webhook Secret v1 rotation remains pending. Retains isolated rollback and historical baselines. |
