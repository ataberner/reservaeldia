...
| Backend configuration isolation | [BACKEND_CONFIGURATION_ISOLATION.md](operations/BACKEND_CONFIGURATION_ISOLATION.md) | Dotenv exposure evidence, per-function secret bindings, source exclusions and manual MP/Maps rotation prerequisites. No remote migration or strict codebase isolation completed. |
| Payments source preparation | [PAYMENTS_CODEBASE_PREPARATION.md](operations/PAYMENTS_CODEBASE_PREPARATION.md) | Local activation of the 102 + 3 partition, isolated discovery, sanitized remote baseline, qualified selectors and per-stage rollback. No deployment performed. |
