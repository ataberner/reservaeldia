...
- **Editor session persistence layer**: `editorSessionPersistence` is the single transport authority for editor-session reads and writes. Draft sessions read/write `borradores/{slug}` through Firestore; template sessions read/write template editor documents through admin callables. `useBorradorSync`, preload, preview re-read, authoring, name saves, and section mutations consume that authority instead of selecting Firestore/callable transport locally. Ordering is serialized through the shared draft-write coordinator for autosave, flush, and section mutation writes.
...
11. The dashboard then re-reads through `readEditorSessionDocument`, so draft preview uses the draft document and template preview uses the template editor document without the preview controller choosing a storage transport directly. Template preview and rollback fallback preview can still overlay the flush-boundary snapshot before local HTML generation, but both are explicitly visual-only. Publishable draft preview uses the re-read mainly for existing-publication link resolution, then delegates prepared HTML generation to the backend.
12. Draft preview generation calls `prepareDraftPreviewRender`, which reads the owned draft on the backend, builds `prepareRenderPayload(...)`, validates it with `validatePreparedRenderPayload(...)`, and generates preview HTML from that same prepared payload. This path is `previewAuthority: "draft-authoritative"`. Blockers return validation without trusted preview HTML; warnings still allow preview. If the rollback/local path is used, it is `previewAuthority: "local-fallback"` and is not publish parity.
   - In a superadmin `adminView=1` draft session, the read-only canvas cannot flush and cannot perform the normal client Firestore re-read. The controller reuses the already authorized `getAdminDraftSnapshot` data for its frontend source step and sends its `ownerUid` to the same prepared preview callable. The backend requires superadmin and matches that owner against the current draft read. Publication-link lookup, publication validation, checkout, and publish actions remain disabled. The source draft stays read-only; the editor user menu may still expose `Crear plantilla`. That action shares the normal draft-template preparation for name, current render payload, authoring state, and generated cover, then calls `adminCreateTemplateFromDraftV1`; it does not flush, repair, write, or delete the viewed draft.
...
- Designer AI does not change that ownership split: its backend interprets and validates a proposal, while its frontend executor delegates mutations to the existing live-authoring owners. The assistant prompt is not a persistence authority.
