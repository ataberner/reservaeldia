...
Current risk:
- `plantillas/boda-clasica/css-boda-clasica.css` and `public/para-diseno/style.css` appear to be duplicate authored/generated copies.
...
2. Define app UI token ownership in Tailwind config or a dedicated token file, preserving existing values.
3. Move auth modal styles from global Bootstrap selectors to app-owned scoped classes.
4. Move landing page styles out of global Bootstrap overrides into route-owned or component-owned styling.
5. Keep or replace `dashboard-invitation-card` with an explicit shared dashboard card primitive.
6. Introduce CSS Modules only for components that need scoped selectors, pseudo-elements, or local keyframes.
7. Define the template CSS source root and artifact generation/copy process.
8. Remove duplicate template/public CSS copies once source/artifact ownership is established.
9. Document and centralize breakpoint names, including Bootstrap compatibility breakpoints.
10. Audit remaining `!important`, broad selectors, and inline static styles after ownership boundaries are in place.
