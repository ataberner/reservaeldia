...
- `src/components/dashboard/home/DashboardHomeStartupLoader.module.css`: component-scoped dashboard entry loader styles. It owns the dashboard-background overlay, heart/halo motion, fade-out, responsive sizing, and reduced-motion fallback. `src/domain/dashboard/homeStartup.js` owns the resource-readiness predicate, and `useDashboardStartupLoaders.js` owns overlay visibility/exit lifecycle.
...
| Dashboard sidebar | `src/components/DashboardSidebar.jsx` | Tailwind in JSX plus inline panel/mobile geometry styles; Assistant tour overlay styles in `src/components/editor/assistantTour/AssistantGuidedTour.module.css` | Owns `[data-dashboard-sidebar="true"]`, `#sidebar-panel`, `data-assistant-tour-target`, and sidebar panel position/height. |
| Dashboard home | `src/components/dashboard/home/DashboardHomeView.jsx` and child components | `DashboardHomeStartupLoader.module.css` for the entry loader; `LandingHero.module.css` for the shared hero; `DashboardPublicationSummarySection.module.css` for the latest publication or latest draft summary; `LandingTemplateShowcase.module.css` through shared carousel primitives for the post-summary carousel block; `LandingHowItWorks.module.css` for the shared step-circle section; `LandingPricing.module.css` for the shared pricing section; `LandingFeatureDetails.module.css` for the shared feature-details section; `LandingShareSection.module.css` for the shared share section; `LandingFooter.module.css` for the shared footer | Keeps home content hidden and inert behind the scoped loader until drafts, publications, templates, and configuration settle, then composes the complete dashboard home. |
...
| `[data-assistant-tour-target]` | `DashboardSidebar.jsx`, `MiniToolbarTabDetallesEvento.jsx` | `AssistantGuidedTour.jsx` | Semantic targets for the non-blocking Assistant guided tour. These replace text-based selectors and must move with the real Assistant controls/fields they identify. |
