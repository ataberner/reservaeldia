...
- `src/components/dashboard/home/DashboardHomeStartupLoader.jsx`
- `src/components/dashboard/home/DashboardHomeStartupLoader.module.css`
...
- `src/domain/dashboard/homeStartup.js`
...
- `src/components/dashboard/home/DashboardPublicationSummarySection.module.css` is the scoped style owner for the dashboard latest-publication summary below the hero and the latest-draft fallback state.
- `src/components/dashboard/home/DashboardHomeStartupLoader.module.css` is the scoped style owner for the dashboard entry background, animated heart/halo, fade-out, responsive sizing, and reduced-motion fallback. `src/domain/dashboard/homeStartup.js` owns the resource-readiness predicate; `DashboardHomeView.jsx` reports that result and `useDashboardStartupLoaders.js` owns overlay visibility/exit lifecycle.
...
- `src/pages/preguntas-frecuentes.module.css` is the scoped owner for the public FAQ route; that route reuses the shared `AppHeader` and `LandingFooter` visual components.
...
| Public FAQ `/preguntas-frecuentes/` | `src/pages/preguntas-frecuentes.js` | `src/pages/preguntas-frecuentes.module.css`, `AppHeader.module.css`, and `LandingFooter.module.css` | Public informational route with route-scoped hero, categorized accordion, and CTA styles. Shared header/footer visuals and responsive navigation behavior remain in their existing owners. |
...
| Dashboard home | `src/components/dashboard/home/DashboardHomeView.jsx` and child components | `DashboardHomeStartupLoader.module.css` for the entry loader; `LandingHero.module.css` for the shared hero; `DashboardPublicationSummarySection.module.css` for the latest publication or latest draft summary; `LandingTemplateShowcase.module.css` through `LandingTemplateCarouselPrimitives.jsx` for the post-summary carousel block; `LandingHowItWorks.module.css` for the shared step-circle section; `LandingPricing.module.css` for the shared pricing section; `LandingFeatureDetails.module.css` for the shared feature-details section; `LandingShareSection.module.css` for the shared share section; `LandingFooter.module.css` for the shared footer | Keeps partial home content hidden and inert until all required sources settle, fades the loader out, then exposes the complete dashboard composition. |
...
| `DashboardHomeStartupLoader.jsx` | Dashboard home entry presentation | `src/components/dashboard/home/DashboardHomeStartupLoader.module.css` | Visual-only loader owner. Uses the existing home readiness gate, keeps the generated invitation/preview loader isolated, and provides a reduced-motion fallback. |
| `DashboardPublicationSummarySection.jsx` | Dashboard home summary for the latest active publication or latest edited draft fallback | `src/components/dashboard/home/DashboardPublicationSummarySection.module.css` | Scoped app UI owner. Shows the draft fallback only when there is no active publication. Avoids `.dashboard-invitation-card*`, global CSS, editor hooks, preview/publish, and generated invitation CSS. |
...
| `[data-assistant-tour-target]` | `DashboardSidebar.jsx`, `MiniToolbarTabDetallesEvento.jsx` | `AssistantGuidedTour.jsx` | Non-blocking guided-tour anchors for real Assistant controls and fields. These are semantic runtime hooks, not styling selectors. |
...
- Dashboard route wrappers in `src/pages/dashboard.js`; the dashboard-home startup presentation is scoped in `DashboardHomeStartupLoader.module.css`.
...
- Dashboard home loading state: dashboard background, centered animated heart and `Preparando tu espacio...` message, hidden/inert partial content, fade-out, and reduced-motion fallback.
- Dashboard home empty state: hero, empty draft/publication/template states, CTA visibility.
- Dashboard home populated state: hero, latest-publication summary or latest-draft fallback, landing-style draft/publication/template carousel block, shared how-it-works section, shared pricing section, shared feature-details section, shared share section, shared footer, template collections anchor, horizontal overflow/fades.
