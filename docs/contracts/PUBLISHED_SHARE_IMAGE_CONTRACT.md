...
4. Select `.inv > .sec:first-child` and hide non-captured sibling sections
   only inside the renderer DOM context.
5. Wait only for images inside the selected first section to load or error.
   Images from hidden later sections must not block `share.jpg`.
6. Wait at least two animation frames after the readiness waits finish.
7. Wait for the selected first section's finite entrance motion to settle.
8. Avoid mutating the stored HTML or the HTML string that will be uploaded.
9. Capture a `1200x630` output.

The image-readiness scope is intentionally narrower than the full published
document: `share.jpg` captures only the first section, so galleries,
decorations, lazy images, GIFs, or other media in later sections are diagnostic
data but are not render blockers for the social image. The renderer must still
report total image counts, first-section image counts, ignored image counts, and
small host/URL samples for pending or failed first-section images.

Before screenshot capture, the renderer must validate that the computed clip is
finite, non-negative, bounded, and normalized to the contractual `1200x630`
region. Invalid clips fail the publish attempt with a controlled renderer error
instead of entering an unbounded screenshot call. Screenshot diagnostics should
include the first-section bounding box, final clip, document scroll dimensions,
viewport/device scale, first-section node/image counts, and small counters for
potentially expensive visual features such as filters, shadows, transforms,
videos, GIFs, SVGs, and CSS background images.
...
The renderer budget must account for both Chromium cold-start work and page
capture work. The default budget is currently 15 seconds, with a reserved final
portion for screenshot capture so finite motion settling cannot consume the full
renderer budget. This budget is evidence-based: cold Chromium executable
resolution and browser launch can consume several seconds before page content is
ready. A timeout, renderer error, missing first section, invalid capture region,
readiness failure, or capture failure must fail the publish attempt in a
controlled way. The previously valid active publication, if any, must remain
safe and must not be replaced by fallback/default share metadata.

For checkout-backed publish attempts, the renderer may persist additive
diagnostic substage metadata on `publication_checkout_sessions`, including
Chromium resolution, browser launch, HTML load, font readiness, image readiness,
layout settling, screenshot, image normalization, Storage save, and Storage
confirmation. These diagnostics do not change the renderer contract, do not
create a second share-image pipeline, and must not be used by the frontend to
infer publish success.
