...
export type PreparedRenderPayload = {
...
export async function prepareRenderPayload(
  ...
  return {
    ...
    contractIssues: Array.isArray(groupAwareState.contractIssues)
      ? groupAwareState.contractIssues
      : [],
    runtimeSupport:
...
export async function preparePublicationRenderState(
  ...
  return prepareRenderPayload(draftData);
}
...
export function validatePreparedRenderPayload(
  ...
  return validatePreparedPublicationRenderState({
    ...
    seccionesFinales: payload.seccionesFinales,
    rawRsvp: payload.draftRenderState.rsvp,
...
export function buildPreviewRenderPayloadFromPreparedPayload(
  ...
  const rsvpConfig = payload.functionalCtaContract.rsvp.config || null;
  const giftsConfig = payload.functionalCtaContract.gifts.config || null;
  const renderState = {
    ...payload.draftRenderState,
    objetos: payload.objetosFinales,
    secciones: payload.seccionesFinales,
  };
  ...
  return {
    ...
    objetos: payload.objetosFinales,
    secciones: payload.seccionesFinales,
    rawRsvp: payload.draftRenderState.rsvp || null,
    rawGifts: payload.draftRenderState.gifts || null,
    rsvpPreviewConfig: rsvpConfig,
    giftPreviewConfig: giftsConfig,
    preparedRenderContract: payload.preparedRenderContract,
    contractIssues: payload.contractIssues,
    runtimeSupport: payload.runtimeSupport,
    functionalCtaContract: payload.functionalCtaContract,
  };
}

export function generateHtmlFromPreparedRenderPayload(
  payload: Pick<
    PreparedRenderPayload,
    "draftRenderState" | "objetosFinales" | "seccionesFinales" | "functionalCtaContract"
  >,
...
export function validatePreparedPublicationRenderState(params: {
  ...
  rawSecciones.forEach((rawSection, index) => {
    ...
    const edgeDecorations =
      finalSection.decoracionesBorde &&
      typeof finalSection.decoracionesBorde === "object"
        ? (finalSection.decoracionesBorde as UnknownRecord)
        : {};

    (["top", "bottom"] as const).forEach((slot) => {
      const safeEdgeDecoration = asRecord(edgeDecorations[slot]);
      if (!Object.keys(safeEdgeDecoration).length) return;
      if (safeEdgeDecoration.enabled === false) return;

      const decorationSrc =
        resolveSectionEdgeDecorationAssetUrl(safeEdgeDecoration) ||
        getString(safeEdgeDecoration.storagePath);
      ...
      pushIssue(
        createIssue({
          severity: "blocking",
          code: "section-edge-decoration-unresolved",
          message: `${sectionLabel} tiene una decoracion de borde sin URL publica resuelta para publish.`,
          sectionId,
          fieldPath: `decoracionesBorde.${slot}.src`,
        })
      );
    });
  });
