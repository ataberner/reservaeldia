...
function buildActiveItem(docItem) {
  const data = docItem.data() || {};
  const nowMs = Date.now();
  const expiresAtMs = toMs(data.vigenteHasta);
  const isExpired = expiresAtMs > 0 && expiresAtMs <= nowMs;
  const isFinalized =
    String(data.estado || "").toLowerCase() === "finalizada" || isExpired;
  const sortMs =
    toMs(data.ultimaPublicacionEn) || toMs(data.publicadaEn) || toMs(data.createdAt);
  ...
  return {
    ...
    url: isFinalized ? "" : String(data.urlPublica || "").trim(),
    borradorSlug: resolveEditableDraftSlug(data),
    isFinalized,
    estado: isFinalized ? "Finalizada" : "Activa",
    publicadaEn: data.publicadaEn || null,
    vigenteHasta: data.vigenteHasta || null,
    finalizadaEn: isFinalized ? data.vigenteHasta || data.finalizadaEn || null : null,
    sortMs,
  };
}
  ...
  useEffect(() => {
    ...
    const fetchPublicaciones = async () => {
      ...
      try {
        ...
        const activeItems = activeSnap.docs.map(buildActiveItem);
  ...
  return (
    <section className="rounded-2xl border border-[#dbe6f6] bg-gradient-to-br from-white via-[#f6f9ff] to-[#eef5ff] p-4 sm:p-6">
      ...
      <p className="mt-1 text-sm text-gray-600">
        Gestiona tus invitaciones activas y revisa las finalizadas.
      </p>
          ...
          <div className={PUBLICADAS_CARD_GRID_CLASS}>
            {cards.map((item) => {
              const editUrl = item.borradorSlug
                ...
                : "";
              const canOpenPublicLink = Boolean(item.url);
              const statusDateLabel = item.isFinalized ? "Finalizada" : "Vigente hasta";
              const statusDateValue = item.isFinalized ? item.finalizadaEn : item.vigenteHasta;

              return (
                <article
                  key={`${item.source}-${item.id}`}
                  className={`group overflow-hidden rounded-2xl border border-gray-200/90 bg-white shadow-[0_2px_8px_rgba(15,23,42,0.06)] transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-[#d9c8f5] hover:shadow-[0_14px_28px_rgba(111,59,192,0.14)] ${canOpenPublicLink ? "cursor-pointer" : ""}`}
                  onClick={() => {
                ...
                >
                  ...
                  <div className="space-y-2 p-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <StatusBadge finalized={item.isFinalized} />
                    </div>
