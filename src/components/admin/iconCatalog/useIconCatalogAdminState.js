...
function parseStatusMessageFromIcon(icon) {
  if (icon?.status === "duplicate") {
    return {
      type: "warning",
      text: "El icono fue archivado por contenido duplicado.",
    };
  }
  ...
  if (icon?.status === "rejected") {
    return {
      type: "error",
      text: "El icono fue rechazado por validacion.",
    };
  }
  ...
  if (validationStatus === "warning") {
    return {
      type: "warning",
      text: "El icono se subio con advertencias de validacion.",
    };
  }
  return {
    type: "success",
    text: "Icono subido y validado correctamente.",
  };
}
...
export function useIconCatalogAdminState({ isSuperAdmin = false } = {}) {
  ...
  const fetchCatalogPage = useCallback(
    async ({ reset = false } = {}) => {
      ...
      let catalogData = await listIconCatalog({
      ...
      if (reset) {
        ...
        if (!hasTotals) {
          try {
            const fallbackTotals = await getIconCatalogTotalsFallback({
              includeArchived: true,
            });
            catalogData = {
              ...(catalogData || {}),
  ...
  const reload = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        setListError(getErrorMessage(error, "No se pudo cargar el catalogo de iconos."));
      } finally {
    ...
    async ({ category, mode }) => {
      ...
      try {
        ...
        if (failedIds.length > 0) {
          ...
          pushFlashMessage(
            "success",
            `${actionLabel} en ${successCount} iconos${skippedCount > 0 ? `, ${skippedCount} sin cambios` : ""}.`
          );
        }
        ...
        (error) => {
          setUploadState({
            ...
            text: getErrorMessage(
              error,
              "No se pudo observar el estado de validacion del icono."
            ),
            validationStatus: null,
            warnings: [],
            errors: [],
          ...
          pushFlashMessage(
            "error",
            "No se pudo consultar el estado de validacion del icono."
          );
          stopProcessingWatcher();
        }
      );
  ...
  const uploadIcon = useCallback(
    async (payload) => {
      ...
      try {
        ...
        if (!iconId) {
          throw new Error("No se pudo obtener el ID del icono subido.");
        }
        ...
        setUploadState({
          phase: "processing",
          iconId,
          text: "Icono subido. Validando archivo en backend...",
          validationStatus: null,
          warnings: [],
          errors: [],
        });
        pushFlashMessage("success", "Icono subido. Esperando validacion del backend.");
        startProcessingWatch(iconId);
        await reload({ silent: true });
        return { ok: true, iconId };
      } catch (error) {
        const message = getErrorMessage(error, "No se pudo subir el icono.");
        setUploadState({
          phase: "error",
          iconId: null,
          text: message,
  ...
  const saveIconEdits = useCallback(
    async ({
      ...
      if (!normalizedId || !current) {
        pushFlashMessage("error", "No se encontro el icono a editar.");
        return { ok: false };
      }
      ...
      if (!nextName) {
        pushFlashMessage("error", "El nombre del icono es obligatorio.");
        return { ok: false };
      }
  ...
  const toggleActivationForIcon = useCallback(
    async (icon) => {
      ...
      try {
        ...
        pushFlashMessage(
          "success",
          nextActive ? "Icono activado." : "Icono desactivado."
        );
      } catch (error) {
        pushFlashMessage(
          "error",
  ...
  const revalidateIconFromGrid = useCallback(
    async (icon) => {
      ...
      try {
        ...
        pushFlashMessage(
          "error",
          getErrorMessage(error, "No se pudo revalidar el icono.")
        );
      } finally {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      pushFlashMessage(
        "error",
        getErrorMessage(error, "No se pudieron cargar mas iconos desde backend.")
      );
    } finally {
