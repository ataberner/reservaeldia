...
export default function useTextEditInteractionController({
  ...
  const entrySelectionAppliedRef = useRef({
    editingId: null,
    applied: false,
  });
  ...
  const focusEditorSelectingAll = useCallback(() => {
    const editorEl = editorRef.current;
    if (!editorEl) return false;
    ...
    const selectedAll = selectAllEditableContent(editorEl);
  ...
  useEffect(() => {
    ...
    if (entrySelectionAppliedRef.current.editingId !== editingId) {
      entrySelectionAppliedRef.current = {
        editingId,
        applied: false,
      };
